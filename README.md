...
ينشئ كل Job مجلدًا مستقلًا تحت `temp/job_<id>/chapter_<number>/`. تُنزّل الصور مباشرة إلى الملفات بدل تجميعها كلها في الذاكرة، وتُحافظ على امتداد الصورة الأصلي عندما يكون معروفًا. ترفع صور كل فصل إلى GoFile بصيغة WebP، ويُرسل رابط مجلد الفصل في Embed نهائي باللغة الإنجليزية. بعد إرسال الرابط، يحذف البوت Embed التقدم وسجل الوظيفة من الذاكرة ومجلد الصور المحلي مباشرة، مع بقاء الملفات محفوظة لدى GoFile. يتطلب استخدام حساب GoFile ثابتًا وضع `GOFILE_TOKEN` في `.env`؛ وإذا تُرك فارغًا يستخدم GoFile حسابًا ضيفًا عند الرفع. توجد واجهة `StorageAdapter` لإضافة مزود تخزين آخر لاحقًا.
