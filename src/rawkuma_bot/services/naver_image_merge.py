...
def merge_images_with_mapping(
    image_paths: list[Path],
    output_dir: Path,
    *,
    max_height: int = NAVER_MERGED_IMAGE_MAX_HEIGHT,
) -> tuple[list[Path], dict[int, list[Path]]]:
    """Merge ordered images and return output files plus source-index mapping.

    The existing Naver function remains unchanged. This companion function is
    used when an HTML document needs to point each source image at one or more
    merged strips after the 14,000px operation.
    """
    if not image_paths:
        return [], {}
    if max_height <= 0:
    ...
    for path in image_paths:
        with Image.open(path) as image:
            max_width = max(max_width, image.width)
    if max_width <= 0:
        raise ValueError("Images have no usable width")

    segments: list[tuple[int, Path, int, int]] = []
    segment_height = 0
    segment_format: str | None = None
    segment_extension: str | None = None
    output_paths: list[Path] = []
    source_outputs: dict[int, list[Path]] = {}
    ...
    def flush_segment() -> None:
        ...
        canvas = Image.new(mode, (max_width, segment_height), background)
        y = 0
        for source_index, source_path, top, height in segments:
            with Image.open(source_path) as source:
                source.load()
                crop = source.crop((0, top, source.width, top + height))
                crop = crop.convert("RGB" if mode == "RGB" else "RGBA")
                canvas.paste(crop, (0, y))
                crop.close()
            y += height
        output_path = output_dir / f".merged_{len(output_paths) + 1:03d}{segment_extension or _extension_for(segment_format)}"
        ...
        canvas.close()
        output_paths.append(output_path)
        for source_index, _, _, _ in segments:
            source_outputs.setdefault(source_index, []).append(output_path)
        segments = []
        segment_height = 0
        segment_format = None
        segment_extension = None
    ...
    for source_index, source_path in enumerate(image_paths):
        ...
        with Image.open(source_path) as source:
            source_width, source_height = source.size
        if source_width <= 0 or source_height <= 0:
            raise ValueError(f"Image is empty: {source_path.name}")
        top = 0
        while top < source_height:
            if segment_format not in {None, image_format}:
                flush_segment()
            ...
            if segment_format is None:
                segment_format = image_format
                segment_extension = source_path.suffix.lower()
            segments.append((source_index, source_path, top, part_height))
            segment_height += part_height
            top += part_height
            if segment_height == max_height:
                flush_segment()
    flush_segment()
    return output_paths, source_outputs
